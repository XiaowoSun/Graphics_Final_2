...
However, there were several times for us that, although the food tasted great, we felt sick or uncomfortable afterwards and thought it might have something to do with the food. Sometimes it can be hard to tell if it is the food poisoning or something else, but if we pay attention to the food safety and health issues while choosing a restaurant, the chance of getting foodborne illness can be elimiated.
...
For `PHONE`, there are some special characters that represents the missing digits in the phone numbers. Some phone numbers have less than 10 digits or simply consist of 10 digits of "0". We converted these values as `NA` at this moment.
```{r, message=FALSE, warning=FALSE}
sp_char <- "_"
...
```
We also filtered out the most recent grades for individual restaurants as a seperate csv file, which can be helpful for us to explore the current grades for restaurants. This seperate dataset contains 25,147 restaurants with grade levels in "A", "B", "C" or "Grading Pending".
```{r, results='hide', message=FALSE, warning=FALSE}
...
```
At the end of this section, we have a cleaned dataset with 382,310 records and 22 variables, which still contains a number of missing values due to the nature of this dataset. We also have a new dataset with 25,147 records of most recent grade for each restaurant and 12 variables, including the basic information of these restaurants.
...
Based on the results by using `mi` package (which was too large to be knit into html file), we have detected 14 patterns of missing values and the top 3 of them are
* `GRADE DATE`, `GRADE`
...
After filtering out the most recent grade for each restaurant, we could take a further look into the current status regarding to the health and safety issues of the restaurants in New York City.

It's comforting to know that over 90 percentage of restaurants inspected in NYC have been graded as "A" and the results do not vary much in each borough, which indicates that the overall results of the most recent inspectation are quite positve.
```{r}
...
There are several cuisine types that all the restaurants graded as "A", such as "Afghan", "Armenian", "Basque", "Chilean", "Czech", "Egyptian", etc. And several cuisine types have relatively high percentage of restaurants graded as "B" or "C", such as "Moroccan", "English", "Iranian", etc.
```{r}
gr_bo <- df_recent %>% 
...
### Other Findings
Although it shows that the majority of the restaurants are graded as "A" at this moment, aftering inspecting the detailed inspection results history of several "Grade A" restaurants, we noticed that there still exists some violations that make us worried as a customer. We do not think it is wise for us to purely reply on the letter grades to make judgement.

# VI. Interactive component

Welcome to the most interesting and inspiring part of this project. The main idea of this part is to allow users themselves to inspect and explore the inspection results of restaurants in New York City. So, we built the shiny app for users to interactive with data and different graphs in a comfortable and flexible way. You can find the app here: https://sheenxx.shinyapps.io/93_graph_proj/.

The app is constructed with three parts. The first part is the Home page section which briefly introduces the project and the team members who contributed to this. The second part is the 2D map section which enables users to explore the most recent grade for the restaurants of their interests with some options. The final part is the Summary Statistics section which displays several types of plots to explore different aspects within the inspectation results.
...
For us, we will just select our favorite cuisine style and enter the zip code where we live. Then the app shows all restaurants of that cuisine style in our inspection result dataset. By clicking those restaurants, we can have more detailed information.
```{r}
knitr::include_graphics('image7.png')
```

If your curiosity is based on the grade of each restaurants, you can change the "Grade of Restaurant" selection. The range of grade is from "A" to "C" along with the "Grade Pending". 

Interestingly, you will actually find out that even those restaurants graded as "A" still have violations that seems to be serious to us customers, such as "Food contact surface not properly washed, rinsed and sanitized after each use and following any activity when contamination may have occurred." So we suggest that we do not determine whether a restaurant is good or not merely based on those grades.
```{r}
...
In the data preprocessing part, we found out that this dataset could be one of the most "hard-to-work-with" raw datasets we have ever encountered. It was generated by the real world activities and intergrated from multiple data source systems, which leads to the issues of missing values, manual input, data inconsistency, regional formats, wrong data types, etc. By dealing with this dataset, we have spent lots of time learning how to handling above issues and realized that during data analysis process under the real-world business context, preprocessing information is as important as creating a good model.

During the data processing part, there are several issues that we are not able to properly resolve:

* Some chain brands that has multiple stores in NYC, while the names for these stores can be slightly different, which is hard to identify and unify the names in an efficient way.

* Most of the missing values can not be filled using current information.

* Some location related variables are wrong in formats or not consistent with each other, which is also hard to identify and unify the names in an efficient way.

* Although the `GRADE` comes from `SCORE`, there are some inconsistency due to the inspection and adjudication process and we can not resolve it without additional information.

There are some we can figure out a better way to present:

* To present the missing value patterns, due to the large size of this dataset, we did not have much choice to plot a better graph.
...
* Before connecting the dataset to the Shiny app, we had to further clean the dataset so that it won’t explode the load of Shiny server. Since our original dataset is huge and it has 383,587 records and 18 variables, we can’t directly put them into map. 

* One of the major problems is that for each restaurant, there are multiple violation records in one inspection date. So we did put all those descriptions into the map as it will make the map very messy. Instead, what we did is to count each type of violations for each restaurants and picked the most frequent one. 

* Since the original dataset doesn’t provide us the exact coordinates of each restaurant, we connected to Google’s API to extract an approximate location. Fortunately , the original dataset has full address of each restaurant so that the location we extracted from Google should be very accurate. 

In the future, we would like to make further attempts into the following aspects:

* For one specific chain brand, for now, we treated its stores as independent business identities. In the future, we can look into a chain brand combining all its stores, and to see if there is any difference in the inspection results among its stores.
